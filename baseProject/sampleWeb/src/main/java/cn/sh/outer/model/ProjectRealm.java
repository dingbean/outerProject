 ...
 * @create Sep 8, 2016 3:15:31 PM
 * @author hp<http://blog.csdn.net/jadyer>
 */
...
//      simpleAuthorInfo.addStringPermissions(permissionList);
        ...
        if(null!=currentUsername && "dk".equals(currentUsername)){
            //添加一个角色,不是配置意义上的添加,而是证明该用户拥有admin角色
            simpleAuthorInfo.addRole("admin");
            //添加权限
            simpleAuthorInfo.addStringPermission("admin:manage");
            System.out.println("已为用户[dk]赋予了[admin]角色和[admin:manage]权限");
...
//      }
        ...
        if("dk".equals(token.getUsername())){
            AuthenticationInfo authcInfo = new SimpleAuthenticationInfo("dk", "dk", this.getName());
            this.setSession("currentUser", "dk");
            return authcInfo;
