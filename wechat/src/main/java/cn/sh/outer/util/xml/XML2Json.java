...
public class XML2Json {
	...
	private static final Log log = LogFactory.getLog(XML2Json.class);
	...
	public JSON read(String xml) {
		...
		try {
			...
			json = processObjectElement(root, JSONTypes.STRING);
	...
	private boolean isArray(Element element, boolean isTopLevel) {
		...
		if (clazz != null && clazz.equals(JSONTypes.ARRAY)) {
			...
			isArray = checkChildElements(element, isTopLevel);
				...
				&& (element.getAttribute("class") != null || element
			...
			if (name.startsWith("@xmlns")) {
				...
				if (colon == -1) {
					...
					if (StringUtils.isBlank(root.getNamespaceURI(prefix))) {
						root.addNamespaceDeclaration(prefix, String
								.valueOf(value));
					}
				}
	...
	private JSON processObjectElement(Element element, String defaultType) {
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (!"FromUserName".equals(child.getQualifiedName())
					&& !"CreateTime".equals(child.getQualifiedName())
					&& !"ArticleCount".equals(child.getQualifiedName())) {
				setValue(jsonObject, child, defaultType);
			}
		}
	...
	private void setValue(JSONArray jsonArray, Element element,
			...
			if (isArray(element, false)) {
				...
				jsonArray.element(simplifyValue(null, processObjectElement(
						element, defaultType)));
			} else {
	...
	private void setValue(JSONObject jsonObject, Element element,
		...
		String key = "ToUserName".equals(element.getQualifiedName()) ? "touser"
				: element.getQualifiedName().toLowerCase();

		if (isArray(element, false)) {
			setOrAccumulate(jsonObject, key, processArrayElement(element,
					defaultType));
		} else if (isObject(element, false)) {
			setOrAccumulate(jsonObject, key, simplifyValue(jsonObject,
					processObjectElement(element, defaultType)));
		} else {
			setOrAccumulate(jsonObject, key,element
					.getValue());
		}
	}
